 ...
 *
 */
public record GameView(int xOrigin, int yOrigin, int height, int width, int squareSize, ImageLoader loader) {
	 ...
	 * @param data    GameData storing each piece of information on the
	...
	public static GameView initGameGraphics(int xOrigin, int yOrigin, int length, GameData data,
		...
		return new GameView(xOrigin, yOrigin, length, data.columns() * squareSize, squareSize, loader);
	}
	 ...
	 */
	private void drawCell(Graphics2D graphics, GameData data, int i, int j) {
		var x = xFromI(i);
		var y = yFromJ(j);
