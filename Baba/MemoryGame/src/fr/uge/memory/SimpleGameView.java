...
public record SimpleGameView(int xOrigin, int yOrigin, int height, int width, int squareSize, ImageLoader loader) {
	...
	private void drawCell(Graphics2D graphics, SimpleGameData data, int i, int j) {
		var x = xFromI(j);
		var y = yFromJ(i);
		Word word = data.getImageAt(i,j);
		if(word != null) {
			var image = loader.image(word.id());
			drawImage(graphics, image, x + 2, y + 2,image.getWidth(),image.getHeight());
		}	
	}
	...
	private void draw(Graphics2D graphics, SimpleGameData data) {
		...
		for (int i = 0; i < data.lines(); i++) {
			for (int j = 0; j < data.columns(); j++) {
				var imageId = data.id(i, j);
				var image = loader.image(imageId);
				drawCell(graphics, data, i, j);
				//drawImage(graphics,image,i,j,image.getWidth(),image.getHeight());
			}
		}
